 ...
 *
 * Thread Safety:
 *   Driver instances are NOT safe for concurrent reads from multiple tasks.
 *   Each instance should be accessed from a single task only. The internal
 *   mutexes protect I2C bus access, but concurrent reads will produce race
 *   conditions in wrap-around detection logic.
 *
 * Power Mode vs Sampling Rate:
 *   When using low power modes, ensure your polling rate matches the sensor's
 *   internal update rate to avoid losing position data:
 *   - LPM1 (5ms internal):   Poll at ≥10ms,  max speed ~180 RPM (50% threshold)
 *   - LPM2 (20ms internal):  Poll at ≥40ms,  max speed ~45 RPM (50% threshold)
 *   - LPM3 (100ms internal): Poll at ≥200ms, max speed ~9 RPM (50% threshold)
 *
 *   Fast movements between samples can exceed wrap detection capability.
 *   Example: 5-second polling with LPM3 → max reliable speed ~360 RPM.
 */

// Define the AS5600 device structure
typedef struct {
    i2c_dev_t i2c_dev;         // I2C device descriptor from i2cdev library
    int32_t accumulated_counts;  // Accumulated counts including wraps (updated on each read)
    int32_t previous_raw_counts; // Last raw count reading (0-4095) for wrap detection
 ...
 *
 * @param dev Pointer to the as5600_t structure to initialize.
 ...
 *
 * Deletes the data mutex and frees the I2C device descriptor.
 *
 * @param dev Pointer to the initialized as5600_t structure.
 * @return ESP_OK on success, or an error code otherwise.
 */
 ...
 *
 * Performs initial communication check and prepares the sensor for on-demand reads.
 * Accumulated counts are tracked internally across reads to handle wrap-around.
 * Must be called after as5600_init_desc.
 *
 * @param dev Pointer to the as5600_t structure with initialized descriptor.
 * @return ESP_OK on success, or an error code otherwise.
 */
 ...
 *
 * Performs an I2C read to get the current sensor value. Automatically tracks
 * accumulated counts internally for wrap-around handling across reads.
 *
 * @param dev Pointer to the initialized as5600_t structure.
 * @param[out] counts Pointer to store the raw counts.
 * @return ESP_OK on success, ESP_ERR_INVALID_STATE if not initialized, or I2C error.
 */
 ...
 *
 * Note: Returns int32_t for precision (no floating-point absorption).
 * Range: ±2.1B counts (±524k revolutions). Convert to revolutions: counts / 4096.0f
 *
 * @param dev Pointer to the initialized as5600_t structure.
 ...
 *
 * @param dev Pointer to the initialized as5600_t structure (can be NULL).
