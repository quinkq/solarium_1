 ...
 *
 * THREAD SAFETY:
 * - Caller must hold xMonitoringMutex (for reading monitoring_data)
 * - Function takes xEnergyMutex (for writing rtc_fluctus_accumulator)
 */
void fluctus_update_energy_accumulator(void)
{
    ...
    // Sanity check: skip if time delta is invalid
    // Note: delta_sec == 0 is valid (multiple calls within same second, no energy accumulated)
    // Max idle interval is 120min (7200s), so threshold set to 7300s (~2h) to catch clock issues
    if (delta_sec < 0) {
        ESP_LOGW(TAG, "Negative time delta: %ld seconds (clock moved backwards?), skipping", (long)delta_sec);
        rtc_fluctus_accumulator.last_sample_time = now;
        xSemaphoreGive(xEnergyMutex);
        return;
    }
    ...
    if (delta_sec > 7300) {
        ESP_LOGW(TAG, "Excessive time delta: %ld seconds (>2h, possible clock issue), skipping", (long)delta_sec);
        rtc_fluctus_accumulator.last_sample_time = now;
        xSemaphoreGive(xEnergyMutex);
        return;
    }

    // Skip accumulation if no time has passed (but don't warn - this is normal)
    if (delta_sec == 0) {
        xSemaphoreGive(xEnergyMutex);
        return;
    }
    ...
    rtc_fluctus_accumulator.pv_energy_wh_accumulator += monitoring_data.solar_pv.power * delta_hour;
    rtc_fluctus_accumulator.battery_energy_wh_accumulator += monitoring_data.battery_out.power * delta_hour;

    // Update hourly peaks
    if (monitoring_data.solar_pv.power > rtc_fluctus_accumulator.pv_peak_w_hour) {
        rtc_fluctus_accumulator.pv_peak_w_hour = monitoring_data.solar_pv.power;
    }
    if (monitoring_data.battery_out.power > rtc_fluctus_accumulator.battery_peak_w_hour) {
        rtc_fluctus_accumulator.battery_peak_w_hour = monitoring_data.battery_out.power;
    }

    // Update daily peaks
    if (monitoring_data.solar_pv.power > rtc_fluctus_accumulator.pv_peak_w_day) {
        rtc_fluctus_accumulator.pv_peak_w_day = monitoring_data.solar_pv.power;
    }
    if (monitoring_data.battery_out.power > rtc_fluctus_accumulator.battery_peak_w_day) {
        rtc_fluctus_accumulator.battery_peak_w_day = monitoring_data.battery_out.power;
    }

    // Check for 15-minute rollover (reset averaging window)
    time_t interval_elapsed = now - rtc_fluctus_accumulator.interval_start_15min;
    if (interval_elapsed >= 900) {  // 900 seconds = 15 minutes
        ESP_LOGD(TAG, "15-minute averaging window rollover (%ld samples collected)",
                 (long)rtc_fluctus_accumulator.sample_count_15min);

        // Reset 15-minute averaging accumulators
        rtc_fluctus_accumulator.interval_start_15min = now;
        rtc_fluctus_accumulator.pv_power_sum_15min = 0.0f;
        rtc_fluctus_accumulator.pv_voltage_sum_15min = 0.0f;
        rtc_fluctus_accumulator.pv_current_sum_15min = 0.0f;
        rtc_fluctus_accumulator.battery_power_sum_15min = 0.0f;
        rtc_fluctus_accumulator.battery_voltage_sum_15min = 0.0f;
        rtc_fluctus_accumulator.battery_current_sum_15min = 0.0f;
        rtc_fluctus_accumulator.sample_count_15min = 0;
    }

    // Update 15-minute averaging sums (all values read from monitoring_data global)
    rtc_fluctus_accumulator.pv_power_sum_15min += monitoring_data.solar_pv.power;
    ...
    rtc_fluctus_accumulator.battery_power_sum_15min += monitoring_data.battery_out.power;
...
bool fluctus_check_hourly_rollover(void)
{
    ...
    if (tm_now.tm_hour != tm_hour_start.tm_hour ||
        ...
        rtc_fluctus_accumulator.pv_produced_wh_day += rtc_fluctus_accumulator.pv_energy_wh_accumulator;
...
void fluctus_midnight_callback(void)
{
    ...
    ESP_LOGI(TAG, "  PV total:         %.1f Wh", rtc_fluctus_accumulator.pv_produced_wh_day);
    ESP_LOGI(TAG, "  Battery consumed: %.1f Wh", rtc_fluctus_accumulator.battery_consumed_wh_day);
    ESP_LOGI(TAG, "  Net energy:       %.1f Wh",
             rtc_fluctus_accumulator.pv_produced_wh_day - rtc_fluctus_accumulator.battery_consumed_wh_day);
    ...
    // Release mutex before telemetry call to avoid recursive mutex deadlock
    // (telemetry_fetch_snapshot -> fluctus_write_to_telemetry_cache -> takes xEnergyMutex)
    xSemaphoreGive(xEnergyMutex);

    // Trigger MQTT publish with current snapshot (BEFORE reset)
    telemetry_fetch_snapshot(TELEMETRY_SRC_FLUCTUS);

    // Re-acquire mutex for reset operation
    if (xSemaphoreTake(xEnergyMutex, pdMS_TO_TICKS(FLUCTUS_MUTEX_TIMEOUT_QUICK_MS)) != pdTRUE) {
        ESP_LOGE(TAG, "Failed to re-acquire energy mutex for daily reset!");
        return;
    }

    // Reset daily counters AFTER snapshot captured
    rtc_fluctus_accumulator.current_day_start = time(NULL);
    rtc_fluctus_accumulator.pv_produced_wh_day = 0.0f;
